  ...
  return (
    <div>
      <div className="relative w-full h-screen overflow-hidden">
        ...
        <div className="absolute top-1/2 transform -translate-y-1/2 w-full bg-background/90 py-8 px-10 md:px-20 backdrop-blur-sm">
          <div className="items-center flex"> {/* Element layout */}
            <Link href="/" className="font-normal text-2xl text-left hover:text-muted-foreground transition-colors">
              {t('navbar.name')}
            </Link>
            <span className="text-sm ml-auto mr-48 text-right space-x-8 hidden xl:block">
              <Link href="/news" className="hover:text-muted-foreground transition-colors">{t('navbar.news')}</Link>
              <Link href="/works" className="hover:text-muted-foreground transition-colors">{t('navbar.works')}</Link>
