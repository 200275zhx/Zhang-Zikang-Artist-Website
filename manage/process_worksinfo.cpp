...
int main() {
    const string inputFilename = "../workinfo.json";
    const string outputFilename = "../output/workdata.json";
    ...
    if (!inputData.empty() && inputData[0].is_array()) {
        auto firstRow = inputData[0];
        if (firstRow.size() == 5 && firstRow[0].get<string>() == "Image Name") {
            // Header row detected, skip the first element.
            startIndex = 1;
            cout << "[DEBUG] Header row detected, skipping it." << endl;
        }
    }
